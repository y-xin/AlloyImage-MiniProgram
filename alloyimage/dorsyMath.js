...
var dorsyMath = {
  ...
  applyMatrix: function (imgData, matrixArr, low) {//对图象信号实行掩模算子变换 low为阈值,滤波运算
    ...
    var template = new dorsyMath.Matrix(matrixArr, matrixSize, 1);
    var tempData = [];
    ...
    for (var i = 0, n = data.length; i < n; i += 4) {
      ...
      var pixelMatrix = new dorsyMath.Matrix(pixelArr, 3, matrixSize);
...
dorsyMath.Matrix.prototype = {
  ...
  plus: function (matrix) {
    ...
    var tempM = new dorsyMath.Matrix([], this.m, this.n);
  ...
  mutiply: function (matrix) {//左乘另一矩阵
    ...
    var tempM = new dorsyMath.Matrix([], this.m, matrix.n);
    for (var i = 0; i < this.m; i++) {
...
dorsyMath.C.prototype = {
  ...
  conjugated: function () {//取共轭
    var tempC = new dorsyMath.C(this.r, -this.i);
    return tempC;
  },
